...
void setup()
{
    ...
    sp.begin(); // Start spotify service

    if (!sp.is_auth())
    {
        USBSerial.print(F("Wait for the user to authenticate: "));
        while (!sp.is_auth())
        {
    ...
    snprintf_P(datestring,
               ...
               timeinfo.tm_hour,
               timeinfo.tm_min);
    drawClock(datestring, getClockDigitColor(timeinfo.tm_hour, timeinfo.tm_min));
    dma_display->flipDMABuffer();
}
